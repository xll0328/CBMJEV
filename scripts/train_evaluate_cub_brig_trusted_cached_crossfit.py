#!/usr/bin/env python3
"""Crossfit BRiG with exact cached risks and private trusted-state kernels."""
...
def source_hashes():
    ...
    names = ("train_evaluate_cub_brig_trusted_cached_crossfit.py",
             "brig_trusted_cached_fit.py", "brig_trusted_ops.py",
             "brig_accelerated_ops.py", "train_evaluate_cub_brig_fast_crossfit.py",
    ...
    return {**{name: file_hash(root / name) for name in names},
            ...
            "contracts_core": file_hash(project / "cbmjev" / "contracts.py")}


def _sidecar(directory):
    return Path(directory) / "trusted_cached_variant_receipt.json"


def verify_trusted_cached_fold(directory):
    directory = Path(directory)
    receipt = read_json(_sidecar(directory))
    unsigned = dict(receipt)
    if (unsigned.pop("receipt_hash", None) != stable_hash(unsigned)
            or receipt.get("format") != "cbmjev-brig-trusted-cached-fold-v1"
        ...
        raise ValueError("trusted-cached BRiG fold receipt/source/checkpoint mismatch")
    return receipt
...
def _write_sidecar(directory, *, command, source_binding, fold_bindings=()):
    directory = Path(directory)
    receipt = {
        "format": "cbmjev-brig-trusted-cached-fold-v1" if command == "train-fold"
                  else "cbmjev-brig-trusted-cached-eval-v1",
        "status": "COMPLETE",
        "algorithm": "terminal-state-cache-and-private-trusted-state-kernels",
...
def main(argv=None):
    ...
    # Inference remains on the validated public GroupQ forward. Only the
    # private fitter calls TrustedTrainingGroupQ.forward_trusted.
    fast.base.GroupQ.forward = accelerated_groupq_forward
    fast.base.fit_brig = fit_brig_trusted_cached

    if args.command == "train-fold":
        result = fast.base.train_fold(args)
        if source_hashes() != before:
            raise ValueError("trusted-cached BRiG sources changed during training")
        _write_sidecar(args.out, command=args.command, source_binding=before)
        verify_trusted_cached_fold(args.out)
    else:
        bindings = [verify_trusted_cached_fold(path) for path in args.fold_models]
        result = fast.base.evaluate(args)
        if (source_hashes() != before or bindings != [verify_trusted_cached_fold(path)
                                                       for path in args.fold_models]):
            raise ValueError("trusted-cached BRiG sources or fold receipts changed")
        _write_sidecar(args.out, command=args.command, source_binding=before,
